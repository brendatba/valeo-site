...
| 9 | Live site replaced | Pages switched from "deploy from main" to GitHub Actions; pushes to `v2` deploy while the PR is open. After merge, set the workflow's branch list back to `main` only. | `.github/workflows/deploy.yml` | Brenda at merge time |
