...
- **Scent imagery:** each fragrance carries `hero` (1200×800), `heroM` (720×480), `thumb` (400×267), `heroVideo` (960×540 loop) and, for the five home-passage worlds, `heroP` / `heroVideoP` / `heroPBg` (portrait). Generated 2026-09-11 with Gemini (stills, style-referenced to the five originals) and Kling via Kie (loops); prompts in `/tmp/valeo-gen/*.mjs` were one-offs — regenerate from `MEDIA-RULING.md`'s prompt shape if a scent is added.
- **Home passage:** the catalog's `passage` array lists the fragrance ids shown behind the pinned jar, in order; the last is the peak. `node scripts/encode-loops.mjs` re-encodes loops dropped in `/tmp/valeo-gen/video/<id>.mp4`.
- **Review loop:** `node scripts/walkthrough.mjs <label>` writes a viewport screenshot pack of a real shopper path to `screens/<label>/`; hand it to reviewers before anything ships.
